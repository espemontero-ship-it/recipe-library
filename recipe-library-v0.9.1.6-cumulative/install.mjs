...
function bumpVisibleVersion(source) {
  return source.replace(/Importer(?:\s+MVP)?\s*[·•-]\s*v\d+(?:\.\d+){1,3}/g, "Importer · v0.9.1.6");
}
...
if (page.includes("RECIPE_LIBRARY_SOURCE_IMAGES_V0915")) {
  page = bumpVisibleVersion(page)
    .replace("RECIPE_LIBRARY_SOURCE_IMAGES_V0915", "RECIPE_LIBRARY_SOURCE_IMAGES_V0916");
  await writeFile(pagePath, page, "utf8");
  console.log("Updated: app/paste/page.tsx from v0.9.1.5");
} else if (!page.includes("RECIPE_LIBRARY_SOURCE_IMAGES_V0916")) {
  ...
  page = bumpVisibleVersion(page);
  page = `${page.trimEnd()}\n\n// RECIPE_LIBRARY_SOURCE_IMAGES_V0916\n`;
  await writeFile(pagePath, page, "utf8");
...
await copyPayload(path.join("supabase", "migrations", "20260719_source_urls_and_images_v2.sql"));
await patchPackageJson();

console.log(`\nRecipe Library v0.9.1.6 cumulative patch installed in:\n${projectRoot}`);
