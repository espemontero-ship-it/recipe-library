...
function bumpVisibleVersion(source) {
  return source.replace(/Importer(?:\s+MVP)?\s*[·•-]\s*v\d+(?:\.\d+){1,3}/g, "Importer · v0.9.1.5");
}
...
if (!page.includes("RECIPE_LIBRARY_SOURCE_IMAGES_V0915")) {
  ...
  page = `${page.trimEnd()}\n\n// RECIPE_LIBRARY_SOURCE_IMAGES_V0915\n`;
...
await copyPayload(path.join("supabase", "migrations", "20260719_source_urls_and_images.sql"));
await patchPackageJson();

console.log(`\nRecipe Library v0.9.1.5 cumulative patch installed in:\n${projectRoot}`);
