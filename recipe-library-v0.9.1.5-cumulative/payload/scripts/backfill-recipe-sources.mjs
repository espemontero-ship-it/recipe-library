...
const TABLE = process.env.RECIPE_TABLE || "recipes_clean_v14_final";
...
async function fetchMetadataImage(sourceUrl) {
  ...
  const response = await fetch(safeSourceUrl, {
    ...
    headers: {
      ...
      "user-agent": "Mozilla/5.0 (compatible; RecipeLibraryBackfill/0.9.1.5)",
    },
...
async function storeImage(imageUrl, sourceUrl, slug) {
  ...
  const response = await fetch(safeImageUrl, {
    redirect: "follow",
    headers: { "user-agent": "Mozilla/5.0 (compatible; RecipeLibraryBackfill/0.9.1.5)" },
