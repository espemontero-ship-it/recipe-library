-- Recipe Library v0.9.1.5
-- Safe cumulative migration from installed v0.9.1.3.
-- Adds source/image provenance fields and recovers exact URLs already present
-- inside raw_source_text. It does not guess URLs from titles or "link in bio".
...
update public.recipes_clean_v14_final as recipe
set
...
create index if not exists recipes_clean_v14_final_source_url_idx
  on public.recipes_clean_v14_final (source_url)
...
drop trigger if exists recipe_library_source_provenance
  on public.recipes_clean_v14_final;
