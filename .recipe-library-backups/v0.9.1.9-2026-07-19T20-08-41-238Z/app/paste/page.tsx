...
function isIngredientLikeLine(value: string) {
  ...
  if (
    /^(?:calories?|protein|carbs?|carbohydrates?|fat|fiber|fibre|servings?|serves|yield|makes|time|recipe by|by|original recipe)\b/i.test(
      line,
    )
  ) {
    return false;
  ...
  return /^(?:\(?\d+(?:[.,]\d+)?(?:\s*[¼½¾⅓⅔⅛⅜⅝⅞])?|[¼½¾⅓⅔⅛⅜⅝⅞]|one\b|two\b|three\b|four\b|five\b|six\b|pinch\b|dash\b|salt\b|pepper\b|cooking spray\b)/i.test(
    line,
  );
}

function isIngredientSubheading(value: string) {
  const line = stripMarkdown(value).trim();
  if (!line.endsWith(":")) return false;

  return !/^(?:calories?|protein|carbs?|carbohydrates?|fat|fiber|fibre|servings?|serves|yield|makes|time|method|directions?|instructions?|preparation)\b/i.test(
    line,
  );
}

function parseSocialIngredientBlock(lines: string[]) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!line) continue;
    if (/^(?:See less|Ver menos)$/i.test(line) || /^#\w/.test(line)) {
      if (started) break;
      continue;
    }

    if (/^(?:method|directions?|instructions?|preparation)\s*:?$/i.test(line)) {
      if (started) break;
      continue;
    }

    if (isIngredientSubheading(rawLine)) {
      const nextLine = lines
        .slice(index + 1)
        .map((candidate) => candidate.trim())
        .find(Boolean);
      if (nextLine && isIngredientLikeLine(nextLine)) {
        started = true;
        continue;
      }
    }

    if (isIngredientLikeLine(rawLine)) {
      started = true;
      ingredients.push(cleanLine(rawLine));
      continue;
    }
...
function inferMainIngredients(title: string, ingredients: string[]) {
  ...
  const candidates = [
    ...
    ["Xanthan Gum", /\bxanthan gum\b|\bgoma xantana\b/],
  ] as const;
...
function parseRecipe(raw: string): ParsedRecipe {
  ...
  const socialHandle = nonEmpty
    .map((line) => stripMarkdown(line).match(/@[A-Za-z0-9_.]+/))
  ...
  const explicitIngredientLines = findSection(
    lines,
    [/^ingredients?\s*:?$/i, /^ingredientes\s*:?$/i],
    [
      /^method\s*:?$/i,
      /^directions?\s*:?$/i,
      /^instructions?\s*:?$/i,
      /^preparation\s*:?$/i,
      /^approximate nutrition\s*:?$/i,
      /^nutrition\s*:?$/i,
      /^serving suggestion\s*:?$/i,
    ],
  )
    .map((line) => line.trim())
    .filter((line) => /^[-*•]\s+/.test(line) || isIngredientLikeLine(line))
    .map(cleanLine);

  const ingredientLines = explicitIngredientLines.length
    ? explicitIngredientLines
    : parseSocialIngredientBlock(lines);
  ...
  return {
    title,
    author: attribution?.[1]?.trim() || socialHandle?.[0] || "",
