...
function isJunkLine(value: string) {
  ...
  return !line || UI_OR_JUNK_LINE.test(line) || CREDIT_LINE.test(line) || /^image\b/i.test(line);
}
...
function parseIngredients(lines: string[]) {
  const start = findSectionStart(lines, SECTION_HEADINGS.ingredients);
  if (start < 0) return [];
  ...
  for (const rawLine of section) {
    ...
    if (/^(?:yield|servings?|serves|makes)\s*:?/i.test(line)) continue;
...
function extractTitle(lines: string[]) {
  const ingredientsIndex = findSectionStart(lines, SECTION_HEADINGS.ingredients);
...
function extractServings(lines: string[]) {
  const direct = normalizeText(lines.join("\n")).match(
    /(?:Yield|Servings?|Serves|Makes|Raciones?)\s*:?\s*(?:about\s*)?(\d+(?:[.,]\d+)?)\s*(?:servings?|raciones?)?/i,
  );
