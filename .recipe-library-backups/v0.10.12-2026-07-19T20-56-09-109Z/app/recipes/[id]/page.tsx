...
export default function RecipePage() {
  ...
  const [inWeeklyPlan, setInWeeklyPlan] = useState(false);
  ...
  useEffect(() => {
    const refreshPlan = () =>
      setInWeeklyPlan(
        getWeeklyPlan().some((item) => item.recipeId === recipe?.id),
      );
    refreshPlan();
  ...
  function toggleWeeklyPlan() {
    if (!recipe) return;
    if (inWeeklyPlan) {
      removeRecipeFromWeeklyPlan(recipe.id);
    } else {
      addRecipesToWeeklyPlan([recipe]);
    }
  }
    ...
    <main className={styles.page}>
      <div className={styles.utilityBar}>
        ...
        <div className={styles.utilityActions}>
          <button
            aria-pressed={inWeeklyPlan}
            className={`${styles.quietButton} ${
              inWeeklyPlan ? styles.weekButtonActive : ""
            }`}
          ...
          >
            ...
            {inWeeklyPlan ? "In this week" : "Add to this week"}
          </button>
