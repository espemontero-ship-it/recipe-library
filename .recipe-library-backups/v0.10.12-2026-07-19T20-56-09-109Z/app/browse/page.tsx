...
type PersonalFilter = "favorite" | "tested" | "weeklyPlan";
...
const PERSONAL_FILTERS: { key: PersonalFilter; label: string }[] = [
  ...
  { key: "weeklyPlan", label: "This week" },
];
...
export default function BrowsePage() {
  ...
  useEffect(() => {
    const refreshPlan = () =>
      setPlannedRecipeIds(getWeeklyPlan().map((item) => item.recipeId));
    refreshPlan();
    return subscribeToWeeklyPlan(refreshPlan);
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    const matches = personalisedRecipes.filter((recipe) => {
      ...
      if (personalFilters.includes("tested") && !recipe.personal.tested) {
        return false;
      }

      if (
        personalFilters.includes("weeklyPlan") &&
  ...
  }, [filters, personalFilters, personalisedRecipes, plannedRecipeIds, query, sort]);
  ...
  function addSelectionToWeek() {
    const selectedRecipes = personalisedRecipes.filter((recipe) =>
      selectedRecipeIds.includes(recipe.id),
    );
    addRecipesToWeeklyPlan(selectedRecipes);
    setSelectedRecipeIds([]);
      ...
      <header className={styles.header}>
        <div className={styles.headerTopline}>
          ...
          >
            ...
            {planningMode ? "Cancel selection" : "Plan this week"}
          </button>
        </div>
          ...
          {filtered.map((recipe) => {
            ...
            return (
              ...
              >
                ...
                {alreadyPlanned && (
                  <span className={styles.plannedBadge}>In this week</span>
                )}
                ...
                <Link
                  ...
                  href={planningMode ? "#" : `/recipes/${recipe.slug}`}
                  onClick={(event) => {
                  ...
                  <div className={styles.body}>
                    <div className={styles.cardTopline}>
                      ...
                      <span className={styles.personalIcons}>
                        ...
                        {alreadyPlanned && (
                          <CalendarDays aria-label="In this week" size={17} />
                        )}
                      </span>
                    </div>
                    <h2>{recipe.title}</h2>
        ...
        <aside className={styles.selectionTray} aria-live="polite">
          <div>
            ...
            <p>
              ...
              <span>Choose several recipes before adding them.</span>
            </p>
          </div>
          <button
            disabled={!selectedRecipeIds.length}
            onClick={addSelectionToWeek}
            type="button"
          >
            Add to this week
          </button>
