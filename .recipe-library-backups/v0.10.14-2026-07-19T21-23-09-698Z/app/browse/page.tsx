...
type PersonalFilter = "favorite" | "tested" | "planning";
...
export default function BrowsePage() {
  ...
  useEffect(() => {
    const refreshPlan = () =>
      setPlannedRecipeIds(getPlanning().map((item) => item.recipeId));
    refreshPlan();
    return subscribeToPlanning(refreshPlan);
  }, []);
  ...
  function addSelectionToPlanning() {
    const selectedRecipes = personalisedRecipes.filter((recipe) =>
      selectedRecipeIds.includes(recipe.id),
    );
    addRecipesToPlanning(selectedRecipes);
    setSelectedRecipeIds([]);
      ...
      {planningMode && (
        <aside className={styles.selectionTray} aria-live="polite">
          ...
          <button
            ...
            onClick={addSelectionToPlanning}
            type="button"
          >
            Add to this week
          </button>
