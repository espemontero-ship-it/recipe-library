...
export default function PlanningPage() {
  ...
  useEffect(() => {
    setPlan(getPlanning());
    return subscribeToPlanning(() => setPlan(getPlanning()));
  }, []);
  ...
  function changeServings(item: PlanningItem, nextValue: number) {
    updatePlanningItem(item.id, {
      servings: Math.max(0.5, Math.round(nextValue * 10) / 10),
    });
  }

  function moveItem(item: PlanningItem, targetWeek: string) {
    if (targetWeek === item.weekStart) return;
    const result = movePlanningItem(item.id, targetWeek);

    if (result.duplicate) {
      const replace = window.confirm(
        "This recipe is already in that week. Replace the existing entry with this one and keep these servings?",
      );
      if (replace) movePlanningItem(item.id, targetWeek, true);
    }
    setCustomMoveItemId(null);
  }

  function confirmClearWeek(group: PlanningGroup) {
    if (
      window.confirm(
        `Remove all ${group.items.length} recipe${group.items.length === 1 ? "" : "s"} from ${group.label.toLowerCase()}?`,
      )
    ) {
      clearPlanningWeek(group.weekStart);
    }
  }
              ...
              <section className={styles.weekSection} key={group.weekStart}>
                <header className={styles.weekHeader}>
                  ...
                  <div className={styles.weekActions}>
                    ...
                    <button
                      onClick={() => confirmClearWeek(group)}
                      type="button"
                    >
                        ...
                        <div className={styles.servingsPanel}>
                          ...
                          <div className={styles.servingsControl}>
                            <button
                              ...
                              onClick={() =>
                                changeServings(item, item.servings - 0.5)
                              }
                              type="button"
                            >
                            ...
                            <select
                              ...
                              onChange={(event) => {
                                if (event.target.value === "__custom__") {
                                  ...
                                  moveItem(item, event.target.value);
                                }
                              }}
                ...
                <footer className={styles.nextStep}>
                  <div>
                    ...
                    <p>
                      ...
                      <span>
                        The shopping list will be generated separately for this
                        week.
                      </span>
                    </p>
                  </div>
