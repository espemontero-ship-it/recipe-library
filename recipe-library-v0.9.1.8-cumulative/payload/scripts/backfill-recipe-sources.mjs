...
let TABLE = process.env.RECIPE_TABLE || "";
const TABLE_CANDIDATES = ["recipes_clean_v14_final", "recipes_clean_v14", "recipes"];
...
async function detectRecipeTable() {
  if (TABLE) return TABLE;

  const { data: configuredTable, error: configError } = await supabase.rpc(
    "recipe_library_active_table",
  );
  if (!configError && typeof configuredTable === "string" && configuredTable.trim()) {
    TABLE = configuredTable.trim();
    return TABLE;
  }
  ...
  throw new Error(
    "Could not detect the recipe table. Run the v0.9.1.8 automatic Supabase migration first, or set RECIPE_TABLE explicitly.",
  );
}
...
async function fetchMetadataImage(sourceUrl) {
  ...
  const response = await fetch(safeSourceUrl, {
    ...
    headers: {
      ...
      "user-agent": "Mozilla/5.0 (compatible; RecipeLibraryBackfill/0.9.1.8)",
    },
...
async function storeImage(imageUrl, sourceUrl, slug) {
  ...
  const response = await fetch(safeImageUrl, {
    redirect: "follow",
    headers: { "user-agent": "Mozilla/5.0 (compatible; RecipeLibraryBackfill/0.9.1.8)" },
...
TABLE = await detectRecipeTable();
console.log(`Using recipe table: public.${TABLE}`);
