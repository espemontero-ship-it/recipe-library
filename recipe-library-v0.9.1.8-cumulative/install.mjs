...
function addTypeFields(source) {
  const hasSourceUrl = source.includes("sourceUrl: string;");
  const hasImageUrl = source.includes("imageUrl: string;");
  if (hasSourceUrl && hasImageUrl) return source;

  const additions = [
    hasSourceUrl ? "" : "  sourceUrl: string;",
    hasImageUrl ? "" : "  imageUrl: string;",
  ].filter(Boolean).join("\n");
...
function addMethodStepHelpers(source) {
  if (source.includes("function normalizeMethodSteps(")) return source;

  const helper = `function methodSectionFromRaw(value: string) {
  const normalized = value
    .replace(/\\r\\n?/g, "\\n")
    .replace(/\\u00a0/g, " ")
    .trim();
  const heading = normalized.match(
    /(?:^|\\n)\\s*(?:#{1,6}\\s*)?(?:preparation|instructions?|directions?|method|steps?|preparaci[oó]n|instrucciones|elaboraci[oó]n|m[eé]todo)\\s*:?\\s*(?:\\n|$)/i,
  );
  return heading && typeof heading.index === "number"
    ? normalized.slice(heading.index + heading[0].length).trim()
    : normalized;
}

function trimMethodTail(value: string) {
  const tail = value.search(
    /(?:^|\\n)\\s*(?:#{1,6}\\s*)?(?:estimated\\s+)?(?:nutrition|macros?|notes?|serving suggestions?|source|fuente|nutrici[oó]n|notas?)\\s*:?\\s*(?:\\n|$)/i,
  );
  return (tail > 0 ? value.slice(0, tail) : value).trim();
}

function stepsFromMarkers(
  value: string,
  marker: RegExp,
  titleFor: (match: RegExpExecArray, index: number) => string,
) {
  ...
  return matches
    .map((match, index) => {
      ...
      return { title: titleFor(match, index), body };
    })
...
function normalizeMethodSteps(
  ...
  const section = trimMethodTail(methodSectionFromRaw(raw));

  const named = stepsFromMarkers(
    section,
    /(?:^|\\n)\\s*(?:step|paso)\\s*(\\d{1,2})\\s*[:.)\\-–—]?\\s*(?:\\n+|\\s+)/gi,
    (match) => "Step " + match[1],
  );
  if (named.length >= 2) return named;

  const numbered = stepsFromMarkers(
    section,
    /(?:^|\\n)\\s*(\\d{1,2})[.)]\\s+/g,
    (match) => "Step " + match[1],
  );
  ...
  const emojiNumbered = stepsFromMarkers(
    section,
    /(?:^|\\n)\\s*([1-9])(?:\\uFE0F?\\u20E3)\\s*/g,
    (match) => "Step " + match[1],
  );
  if (emojiNumbered.length >= 2) return emojiNumbered;

  return existing;
}`;

  if (source.includes("function appendMissingUrls(")) {
    return insertAfterFunction(source, "appendMissingUrls", helper);
  }
  return insertAfterFunction(source, "cleanLine", helper);
}

function normalizeExtractedMethod(source) {
  if (source.includes("nextRecipe.method = normalizeMethodSteps(raw, nextRecipe.method);")) {
    return source;
  }
  ...
  return replaceRequired(
    ...
    `$1\n    nextRecipe.method = normalizeMethodSteps(raw, nextRecipe.method);`,
    "the parsed recipe assignment",
  );
}

function addParsedValues(source) {
  const hasSourceUrl = /sourceUrl:\s*extractSourceUrl\(raw\)/.test(source);
  const hasImageUrl = /imageUrl:\s*""/.test(source);
  if (hasSourceUrl && hasImageUrl) return source;

  const additions = [
    hasSourceUrl ? "" : "    sourceUrl: extractSourceUrl(raw),",
...
function addReviewFields(source) {
  const hasSourceUrl = source.includes('{ label: "Source URL"');
  const hasImageUrl = source.includes('{ label: "Cover image"');
  if (hasSourceUrl && hasImageUrl) return source;

  const additions = [
    hasSourceUrl ? "" : '      { label: "Source URL", confidence: confidence(parsed.sourceUrl) },',
    hasImageUrl ? "" : '      { label: "Cover image", confidence: confidence(parsed.imageUrl) },',
  ].filter(Boolean).join("\n");
...
function addSaveFields(source) {
  const hasSourceUrl = /originalUrl:\s*parsed\.sourceUrl/.test(source);
  const hasImageUrl = /image:\s*parsed\.imageUrl/.test(source);
  if (hasSourceUrl && hasImageUrl) return source;

  const additions = [
    hasSourceUrl ? "" : "        originalUrl: parsed.sourceUrl,",
    hasImageUrl ? "" : "        image: parsed.imageUrl,",
  ].filter(Boolean).join("\n");

  return replaceRequired(
    source,
    /(publication:\s*parsed\.publication,)/,
    `$1\n${additions}`,
...
function addSourceFieldsUi(source) {
  const hasSourceUrl = source.includes('htmlFor="sourceUrl"');
  const hasImageUrl = source.includes('htmlFor="imageUrl"');
  if (hasSourceUrl && hasImageUrl) return source;
  ...
  const sourceUi = hasSourceUrl ? "" : `          <div className={styles.field}>
...
`;
  ...
  const imageUi = hasImageUrl ? "" : `          <div className={styles.field}>
            <div className={styles.fieldLabel}>
...
`;
  ...
  return `${source.slice(0, start)}${sourceUi}${imageUi}${source.slice(start)}`;
}

function addImagePreviewUi(source) {
  if (source.includes('data-recipe-cover-preview="true"')) return source;

  const imageInput = /(value=\{parsed\.imageUrl\}\s*\n\s*\/\>)/;
  return replaceRequired(
    source,
    imageInput,
    `$1
            {parsed.imageUrl ? (
              <div data-recipe-cover-preview="true" style={{ marginTop: "0.75rem" }}>
                <img
                ...
                />
                <p style={{ margin: "0.5rem 0 0", opacity: 0.72 }}>Image found</p>
              </div>
            ) : (
              <p data-recipe-cover-preview="true" style={{ margin: "0.5rem 0 0", opacity: 0.72 }}>
                No image found yet
              </p>
            )}`,
    "the cover image input",
  );
}

function bumpVisibleVersion(source) {
  return source.replace(/Importer(?:\s+MVP)?\s*[·•-]\s*v\d+(?:\.\d+){1,3}/g, "Importer · v0.9.1.8");
}
...
page = page
  .replace(/RECIPE_LIBRARY_SOURCE_IMAGES_V0915/g, "RECIPE_LIBRARY_SOURCE_IMAGES_V0918")
  .replace(/RECIPE_LIBRARY_SOURCE_IMAGES_V0916/g, "RECIPE_LIBRARY_SOURCE_IMAGES_V0918")
  .replace(/RECIPE_LIBRARY_SOURCE_IMAGES_V0917/g, "RECIPE_LIBRARY_SOURCE_IMAGES_V0918");
page = addTypeFields(page);
page = addUrlHelpers(page);
page = addMethodStepHelpers(page);
...
page = normalizeExtractedMethod(page);
page = addSaveFields(page);
page = addPasteCapture(page);
page = addSourceFieldsUi(page);
page = addImagePreviewUi(page);
page = bumpVisibleVersion(page);
if (!page.includes("RECIPE_LIBRARY_SOURCE_IMAGES_V0918")) {
  page = `${page.trimEnd()}\n\n// RECIPE_LIBRARY_SOURCE_IMAGES_V0918\n`;
}
await writeFile(pagePath, page, "utf8");
console.log("Patched and repaired: app/paste/page.tsx");
...
console.log(`\nRecipe Library v0.9.1.8 cumulative patch installed in:\n${projectRoot}`);
console.log("Next: restart with npm run dev and open http://localhost:3000/paste.");
console.log("Do not rerun Supabase SQL if the automatic migration already returned active_table.");
